...
title Update Next-Trainer from Release
...
echo ========================================
echo   Next-Trainer Release Update
echo   从 GitHub Release 更新整合包
echo ========================================
echo.
echo This downloads the latest Next-Trainer-v*.7z from Releases,
echo merges Next-Trainer code, and keeps your models/output/logs.
...
if not exist "%PROJECT_DIR%\gui.py" (
    echo [Error] Next-Trainer not found / 未找到 Next-Trainer
    pause
    exit /b 1
)
...
set "PS_SCRIPT=%PORTABLE_ROOT%Next-Trainer\scripts\portable\update_from_release.ps1"
...
if not "%RC%"=="0" (
    ...
    echo You can still use Update-Next-Trainer.bat ^(git^) or download 7z manually:
...
echo   PowerShell: %PORTABLE_ROOT%Next-Trainer\scripts\portable\update_from_release.ps1
echo.
goto :eof
