...
``sys.modules`` (including ``mikazuki.engines.anima_fast.service_resolver``) so
...
``mikazuki.engines.anima_fast.service_resolver`` (e.g.
``tests/test_anima_fast_backend.py``) fails during collection with::
    ...
    'mikazuki.engines.anima_fast.service_resolver' (unknown location)
