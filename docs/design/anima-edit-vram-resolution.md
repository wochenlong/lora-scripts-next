...
3. [机制：双参考为何更吃显存](#3-机制双参考为何更吃显存)
   - [3.1 T 槽位与 ref / output 关系（FAQ）](#31-t-槽位与-ref--output-关系faq)
4. [仓库内配置对照](#4-仓库内配置对照)
5. [其它观测记录](#5-其它观测记录)
6. [显存优化手段](#6-显存优化手段)
...
### 3.1 T 槽位与 ref / output 关系（FAQ）

**问：模型能否靠 `T=0/1/2/3` 区分任务，并把 ref 与 input/output 关系建起来？**

**答：能区分「槽位角色」，但不是任意任务 ID。**

| 槽位 | 本仓库 Edit 协议中的角色 |
|:----:|--------------------------|
| **T=0** | 加噪的 **target**（要学回去 / 推理时要 denoise 的图） |
| **T=1** | 参考图 **ref1**（干净 VAE latent，条件帧） |
| **T=2** | 参考图 **ref2**（双参考时；三参考则再占 T=3） |

模型**没有**单独的 `ref_id` / `role` token；靠下面几件事绑关系：

1. **固定顺序**：`torch.cat(..., dim=2)` 时 target 总在 T=0，参考图依次排在后面（见 `anima_train_network.py`）。
2. **内容差异**：仅 T=0 加噪；T≥1 为干净 conditioning latent。
3. **监督**：loss 只取 `model_pred[:, :, :1]`，即只监督 T=0。
4. **位置编码**：DiT 使用 **3D RoPE**（及可选 T 轴 learnable pos），不同 `T` 在注意力里是不同时空位置。
5. **文本**：caption 通常描述 target 侧期望效果。

因此：**「ref → 要生成的 output」** 是靠 **0 号槽 = 输出、其余槽 = 条件** 这套协议学出来的，而不是给每个 T 贴显式任务标签。

**局限（实现者需注意）**：

- **ref1 vs ref2 的语义**（例如哪张管表情、哪张管身份）没有结构化标注，只靠 **T=1 / T=2 位置** + 数据里 **子目录文件名排序**（`1.png`、`2.png`）+ 图文对；交换 ref 顺序可能与训练分布不一致。
- **不宜**把 `T` 当成可随意重排的「任务编号」；换 conditioning 拼接方式后，槽位语义需重训或重测。
- 全局 diffusion **timestep** 在训练里多为 `(B,1)` 广播到各槽；区分 ref/target 主要靠 **噪声/干净内容 + 槽位索引**，而非 per-slot 不同 `t`。

> 换实现者若不再沿 `dim=2` 拼接，除 §2 显存外，本节槽位语义也需一并重新约定与验证。
...
| 2026-05-27 | §3.1：T 槽位与 ref/output 关系 FAQ（开发者常见问题） |
