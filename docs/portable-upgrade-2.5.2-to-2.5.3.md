...
**请 v2.5.2 用户升级到 v2.5.3**，仅用手动进 `Next-Trainer` 跑 `gui.py` 属于临时规避，不能替代正式升级。
...
若你的 v2.5.2 整合包是通过官方渠道获得、且 `Next-Trainer/` 内存在 `.git`：

1. 关闭正在运行的 Next-Trainer 窗口。
2. 双击 **`Update-Next-Trainer.bat`**，等待拉取完成。
...
2. 进入 `Next-Trainer` 目录，执行：`gui.py`（**不要**加 `--skip-prepare-environment`）
3. 等待依赖补全完成后，以后仍建议升级到 v2.5.3 正式包。

## 如何确认已修复

- 启动日志 `next-trainer-log.txt` 中可见 `[setup] Verifying embedded dependencies`；若曾缺依赖，会出现 `[Repair] Incomplete dependencies detected` 并跑 setup。
- 训练页点击「开始训练」不再出现「无法连接训练端」（在模型与数据路径正确的前提下）。

## 反馈

仍失败请附带整合包根目录 **`next-trainer-log.txt`**，并在 [Issue #54](https://github.com/wochenlong/lora-scripts-next/issues/54) 或新 Issue 中说明是否已从 v2.5.2 升级、侧栏显示的版本号。
