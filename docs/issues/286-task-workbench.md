...
> 范围：Issue #286 的 P0（1-3）、P1（4-7）与 #8 插队；#9-11 经评估暂缓（理由见表）。
> 另含两个实测修复：终态任务历史持久化（全部落盘、无自动清理）、停训杀整棵进程树（修僵尸占卡与队列卡死）。
...
| 8 | 队列优先级/插队 | ✅ 完成 | 「提到最前」（QUEUED/待确认均可，阶段组整体移动，排在 running 之后） |
| 9 | 多任务 loss/预览对比 | ⏸️ 暂缓 | 本质是产物对比，应做在产物/输出管理视角，不适合堆在任务面板 |
| 10 | 备注、标签、置顶 | ⏸️ 暂缓 | 价值不明确；output_name + 筛选搜索已够定位，真需要再做 |
| 11 | 维护/训练任务分区 | ⏸️ 暂缓 | 前提是先把打标等维护任务正确标记进任务体系（kind/lane 标注不全），属独立工作，超出本 PR 范围 |
