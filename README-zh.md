...
<p align="center">
  <strong>本地模型训练管理器</strong><br />
  熟悉的训练操作 · 多引擎 · 原生面向 Agent<br />
  <sub>面向专业玩家与平台 · GitHub 仓库名 <code>lora-scripts-next</code></sub>
</p>
...
**Next Trainer** 不是又一个「把训练脚本包一层网页」的工具。  
它是给专业玩家和平台用的**本地模型训练管理器**：还原你已经熟悉的训练操作，把多引擎收进同一套工作台，并且从一开始就按 **Agent 能直接协作** 来设计。

你还是在 Windows 本机、NVIDIA 显卡上训 LoRA 或全量微调。  
差别在于：开训、盯任务、管数据集、换引擎，不再散落在脚本窗口和临时网页里，而是落在一个能长期用的管理面里。

底层仍站在成熟训练栈上：主路径基于 [kohya-ss/sd-scripts](https://github.com/kohya-ss/sd-scripts)；训 Krea 2 时可按需接入 [musubi-tuner](https://github.com/kohya-ss/musubi-tuner)。  
...
一句话：从打标、改标签，到选模型开训、盯进度、看日志，尽量在同一个本地管理器里完成。
...
如果你已经会训，却受够了「每个引擎一套环境、每次开训一堆窗口、人和脚本各说各话」，Next Trainer 就是为这件事做的。

1. **操作还是熟悉的那一套**  
   选模型、填参数、导入 TOML、开训、看预览——路径贴近秋叶系习惯，上手成本低，不用为了换壳重新学一遍。

2. **多引擎，一个管理面**  
   Kohya 是基线；Anima Fast、Musubi 等可选引擎在设置页按需安装与切换。平台或工作室不必为每个后端再维护一套独立 UI。

3. **原生面向 Agent**  
   配置可导入导出，任务与日志可被程序读取，工作台结构稳定。人可以点界面；Agent 也可以按同一套流程协作，而不是事后再硬接一层自动化。

4. **任务过程看得见**  
   状态、日志、预览和 Loss 收在任务页。训起来之后，盯盘不必再开一堆外部窗口。

5. **给认真训模型的人，也给要接平台的人**  
   个人用整合包就能跑。工作室、平台和跟试验线的开发者，可以基于源码、`main` / `dev` 做二次集成与长期维护。

它不是云端一键平台，也不假装替代所有专用工具。  
它要解决的是：本地模型训练这件事，该有一个专业、可扩展、人和 Agent 都能用的管理器。
