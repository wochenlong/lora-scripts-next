...
from guide_html_shared import GUIDE_HASH_SCRIPT, GUIDE_HTML_BODY, GUIDE_PAGER_CSS
...
def write_guide_assets() -> None:
    ...
    # SSR page from changelog template (do NOT blanket-replace changelog.html → guide.html:
    # that corrupts chunk hashes into guide.html.e5f6a7b8.js which do not exist).
    ...
    g = g.replace("guide.html.e5f6a7b8.js", "guide.html.c3f4a902.js")
    g = g.replace("guide.html.a1b2c3d4.js", "guide.html.b8e2d701.js")
    g = g.replace("更新日志", "新手上路", 2)
    start = g.find('<div class="sd-changelog">')
    if start < 0:
        start = g.find("<h2 id=")
    b = g.find("</div><!--[--><!--]--></div><footer")
    if start < 0 or b < 0:
        raise SystemExit("guide.html body markers not found")
    g = g[:start] + GUIDE_HTML_BODY + g[b:]
    g = g.replace("<title>更新日志 | SD 训练 UI</title>", "<title>新手上路 | SD 训练 UI</title>")
    if GUIDE_HASH_SCRIPT not in g:
        g = g.replace("</head>", GUIDE_HASH_SCRIPT + "</head>")
