Schema.intersect([
    Schema.object({
        ...
        lora_type: Schema.union(["lora", "dora", "lokr", "tlora", "lora_fa", "vera", "loha"]).default("lora").description("适配器类型。常规训练建议选择 LoRA；DoRA 为独立入口，后端自动使用 LyCORIS DoRA"),
    ...
    Schema.intersect([
        ...
        Schema.union([
            ...
            Schema.object({
                ...
                network_module: Schema.const("lycoris.kohya").default("lycoris.kohya").hidden(),
                lycoris_algo: Schema.const("lora").default("lora").hidden(),
                dora_wd: Schema.const(true).default(true).hidden(),
                dropout: Schema.number().step(0.01).min(0).max(1).description("DoRA dropout 概率"),
                rank_dropout: Schema.number().step(0.01).min(0).max(1).description("rank dropout 概率"),
                module_dropout: Schema.number().step(0.01).min(0).max(1).description("module dropout 概率"),
                pissa_init: Schema.boolean().hidden(),
                lokr_factor: Schema.number().hidden(),
                full_matrix: Schema.boolean().hidden(),
                use_cp: Schema.boolean().hidden(),
                use_scalar: Schema.boolean().hidden(),
                decompose_both: Schema.boolean().hidden(),
                bypass_mode: Schema.boolean().hidden(),
                rank_dropout_scale: Schema.boolean().hidden(),
                conv_dim: Schema.number().hidden(),
                conv_alpha: Schema.number().hidden(),
                network_dropout: Schema.number().hidden(),
            }),
