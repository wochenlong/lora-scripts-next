...
class ExtensionLayout:
    ...
    def package_dir(self) -> Path:
        return self.source / "src" / "musubi_tuner"

    @property
    def train_script(self) -> Path:
        return self.source / "krea2_train_network.py"

    @property
    def pyproject(self) -> Path:
        return self.source / "pyproject.toml"
...
def default_layout(root: Path | None = None) -> ExtensionLayout:
    ...
    return ExtensionLayout(base / "extensions" / "musubi_tuner")
...
def _missing_runtime_files(layout: ExtensionLayout) -> list[str]:
    required = (
        (layout.pyproject, "pyproject.toml"),
        (layout.package_dir, "src/musubi_tuner"),
        (layout.train_script, "krea2_train_network.py"),
    )
