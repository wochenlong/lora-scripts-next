...
- `start` / `stop` 仅限本机（base-url 为 127.0.0.1/localhost）；远程请 ssh 到目标机器再执行。
...
python3 $NT search learning_rate                 #    或：跨全部 schema + reference/ + 仓库 docs/ 聚合检索关键字
...
python3 $NT preview <task_id>                    # 下载最新预览图，打印本地路径（--all 下全部）
...
- **schemas 原文很大**：优先用 `params <page>`（紧凑字段表，公共字段在 `shared` 页）
  和 `search <关键字>`（跨 schema + SKILL.md + reference/ + 仓库 `docs/*.md` 聚合检索，一次定位参数含义与文档出处）。
- 参数含义不清时先读 `reference/` 文档或仓库 `docs/`（`search` 命中后按行号 Read），再向用户提问。
...
- `gpu_ids` 字段可能触发后端 500（非 JSON 响应）；单卡环境建议不传该字段，让后端默认分配
  （nt.py 提交前也会对此给出 client_warnings）。
- Anima Fast 训练时长 epoch/steps **互斥**（`training_duration_mode`，#349）：导入同时含两个字段的
  旧 TOML 时按 Epoch 优先并提示；API 提交自己保证只给一个。
- `torch_compile` 与 `attn_mode=torch`（或留空）组合会被后端拒绝（#336）；要编译就显式选
  xformers/sageattn/flash。
- Anima Fast `fast_variant=tlora` 与 `network_train_unet_only=false` 不兼容，后端会拒；
  关闭 unet_only（训 text encoder）时也不能同时开文本编码缓存。
- 数据集 toml 各引擎**不通用**：kohya 系（anima-lora 等）不收 `subsets[].recursive`、`cache_dir`
  （这些是 anima-fast 字段）——`dataset-validate` 会点名。anima-fast 的 `validation_split_num`
  默认已为 0（#339），小数据集不会再被静默扣验证图。
