...
async function refresh(reportError = true) {
  try {
    status.value = await enginesApi.status("musubi")
...
async function install(forceReinstall = false) {
  try {
    await ElMessageBox.confirm(
      forceReinstall ? t("settings.engines.confirm.reinstall") : t("settings.engines.confirm.install.musubi"),
    ...
    logs.value = []
    progress.value = 0
    const data = forceReinstall ? await enginesApi.repair("musubi") : await enginesApi.install("musubi")
...
<template>
  ...
  <div v-else class="fast-page" :class="{ bare }">
    ...
    <aside class="install-console">
      <header>
        <span>{{ phase || t("musubiGate.consoleIdle") }}</span>
        <b>{{ progress }}%</b>
      </header>
      <div class="install-progress"><i :style="{ width: `${progress}%` }" /></div>
      <pre>{{ logs.length ? logs.join("\n") : t("musubiGate.consoleWaiting") }}</pre>
    </aside>
  </div>
</template>
